...
const EDGE_TRANSLATE_PDF_URL_RE = /^(blob|file|https?|chrome-extension|moz-extension|safari-web-extension):/i;
const EDGE_TRANSLATE_ENCODED_PDF_URL_RE = /^(blob|file|https?|chrome-extension|moz-extension|safari-web-extension)%3a/i;
function decodeEdgeTranslateViewerFileParam(value) {
  ...
  for (let i = 0; i < 2; i++) {
    ...
    if (!EDGE_TRANSLATE_PDF_URL_RE.test(decoded) && !EDGE_TRANSLATE_ENCODED_PDF_URL_RE.test(decoded)) {
      break;
    }
    current = decoded;
  }
...
const defaultOptions = {
  ...
  workerSrc: {
    value: "../build/build/pdf.worker.mjs",
    kind: OptionKind.WORKER
  }
};
{
  ...
  defaultOptions.sandboxBundleSrc = {
    value: "../build/build/pdf.sandbox.mjs",
    kind: OptionKind.VIEWER
  };
...
const PDFViewerApplication = {
  ...
  async run(config) {
    ...
    const originalFileParam = file;
    file = decodeEdgeTranslateViewerFileParam(file);
    if (file !== originalFileParam) {
      try {
